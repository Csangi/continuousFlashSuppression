...
public class UIManager : MonoBehaviour
{
    ...
    public void uploadExperiment(String[] holderPath, int numberCondition, int numberBlock)
    {
        ...
        int delay = 0;              //delay till next img. nothing shows in this time. in the beginning
        int maxOp = 0;              //the time in ms to get to max opacity. 
        ...
        if (path.Length != 0)
        {
                        ...
                        if (counter > 0)            //this removes the first line of the csv, which will be the title of the column
                        {
                            ...
                            switch (i)                          //switch statement depanding on the column of the csv
                            {
                                    ...
                                    {                           //we will wait until the first column, third row (first row of csv is ignored so second row of what is read) 
                                        switch (v)              //and we will enter the values of the privous row before we read the next values in. 
                                        {
                                            case 0:
                                                ...
                                                break;
                                            case 1:
                                                //MondTrial(bool rand, int blk, Version type, string img, int dur, int flshdur, float opa, int dly, int TTRO)
                                                experiment.conditions[numberCondition].blocks[numberBlock].addTrial(new MondTrial(trialRand, block0, img, duration, flash, opacity, delay, maxOp, mond));
                                                mond = 0;       //set the default value of mond to 0 if one is not uploaded then this will be input
                                                break;
                                            case 2:
                                                //FlashMondTrial(bool rand, int blk, string img, int dur, int flshdur, float opa, int dly, int TTRO, int mond)
                                                experiment.conditions[numberCondition].blocks[numberBlock].addTrial(new FlashMondTrial(trialRand, block0, img, duration, flash, opacity, delay, maxOp, mond));
                                                mond = 0;       //set the default value of mond to 0 if one is not uploaded then this will be input
                                                break;
                                            case 3:
                                                //MaskTrial(bool rand, int blk, string img, int dur, int flshdur, float opa, int dly, int TTRO, int mond)
                                                experiment.conditions[numberCondition].blocks[numberBlock].addTrial(new MaskTrial(trialRand, block0, img, duration, flash, opacity, delay, maxOp, mask));
                                                break;
                                            case 4:
                                            ...
                                            case 5:
                                                ...
                                                up = "up"; down = "down"; left = "left"; right = "right";       //if values are not uploaded for the responses these will be the default values entered
                                                break;
                                        }
                                    }
                                ...
                                case 1:                         //condition random
                                    if (result == 1)                                            //one is true, zero is false  
                                    {
                                        experiment.conditions[numberCondition].random = true;   //this will technically means that the last input is what really matters but ¯\_(ツ)_/¯
                                    }
                                    else
                                    {
                                        experiment.conditions[numberCondition].random = false;
                                    }
                                    break;
                                case 2:                         //block
                                    ...
                                    break;
                                case 3:                         //block.random
                                    if (result == 1)                        //same logic as the condition rand
                                    {
                                        experiment.conditions[numberCondition].blocks[numberBlock].random = true;
                                    }
                                    else
                                    {
                                        experiment.conditions[numberCondition].blocks[numberBlock].random = false;
                                    }
                                    break;
                                case 4:                         //trial type
            ...
            switch (v)          //extra switch statement for the last thing uploaded
            {
                case 0:
                    //Instruction(bool rand, int blk, Version type, string img, string imgpth, int dur)
                    experiment.conditions[numberCondition].blocks[numberBlock].addTrial(new Instruction(trialRand, block0, img, duration));
            ...
            int numOfMonds = 0;
            foreach (Mondrian m in experiment.Mondrians)
                if (m.isUsed)
                    ++numOfMonds;
            if (numOfMonds >= 6)
            {
    ...
    {
        string imagePath = path;                                                //its called image path because I copied my code from the image uploader
        ...
        Debug.Log(imagePath);
        imagePath += "mond.csv";                                                //the mond file should always be in the same directory as the original csv in a folder called mondrian
        mondPath = imagePath;                                                   //now we can find the mond csv
                        ...
                        if (counter > 0)
                        {
                            ...
                            switch (i)
                            {
                                ...
                                case 3:
                                    minW = result;
                                    break;
                                case 4:                         //maximum width
                                    maxW = result;
                                    break;
                                case 5:                         //minimum height
                                    minH = result;
                                    break;
                                case 6:                         //maximum height
                                    maxH = result;
                                    break;
                                case 7:                         //density
                                    Density = result;
    ...
    public void uploadImages()
    {
        ...
        for (int i = 0; i < experiment.numberOfConditions; i++)                                     //loop through conditions
        {
            ...
            for (int j = 0; j < experiment.conditions[i].numberOfBlocks; j++)                               //loop blocks
            {
                blockIndex = experiment.conditions[experiment.index[i]].index[j];
                for (int k = 0; !successfulRead && k < experiment.conditions[i].blocks[j].numberOfTrials && uploadSuccessfull; k++)      //loop trials
                {
